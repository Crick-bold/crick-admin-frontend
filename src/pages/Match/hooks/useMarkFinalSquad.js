...
const useMarkFinalSquads = ({ matchId, getMatchById, squad1, squad2 }) => {
  ...
  const isSquadsValidated = () => {
    const team1playing11 = squad1?.players?.filter((player) => {
      return player?.selected;
    });
    const team2playing11 = squad2?.players?.filter((player) => {
      return player?.selected;
    });
  ...
  const markFinalSquads = () => {
    if (isSquadsValidated()) {
      markSquads().then((res) => {
        setLoading(false);
        setData(res);
        getMatchById(matchId);
      });
    } else alert("Squads are not in format");
  };
